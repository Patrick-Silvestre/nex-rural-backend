package com.agromach.controller;

import com.agromach.dto.ProdutoMarketplaceDto;
import com.agromach.service.ProdutoMarketplaceService;
import com.agromach.util.ApiPaths;
import io.swagger.v3.oas.annotations.tags.Tag;
import jakarta.validation.Valid;
import java.util.List;
import lombok.RequiredArgsConstructor;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.security.access.prepost.PreAuthorize;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * Controller REST de ProdutoMarketplace. Recebe requisicoes HTTP, valida dados de entrada e delega regras para a camada de servico.
 */
@RestController
@RequestMapping(ApiPaths.PRODUTOS)
@RequiredArgsConstructor
@Tag(name = "Produtos Marketplace")
public class ProdutoMarketplaceController {

    private final ProdutoMarketplaceService produtoMarketplaceService;

    @PostMapping
    @PreAuthorize("hasAnyRole('USER','ADMIN')")
    public ResponseEntity<ProdutoMarketplaceDto.ProdutoMarketplaceResponse> create(
        @Valid @RequestBody ProdutoMarketplaceDto.ProdutoMarketplaceRequest request
    ) {
        return ResponseEntity.status(HttpStatus.CREATED).body(produtoMarketplaceService.create(request));
    }

    /**
     * Retorna os dados solicitados conforme os criterios da operacao.
     */
    @GetMapping
    @PreAuthorize("permitAll()")
    public ResponseEntity<List<ProdutoMarketplaceDto.ProdutoMarketplaceResponse>> findAll() {
        return ResponseEntity.ok(produtoMarketplaceService.findAll());
    }

    /**
     * Retorna os dados solicitados conforme os criterios da operacao.
     */
    @GetMapping("/{id}")
    @PreAuthorize("permitAll()")
    public ResponseEntity<ProdutoMarketplaceDto.ProdutoMarketplaceResponse> findById(@PathVariable Long id) {
        return ResponseEntity.ok(produtoMarketplaceService.findById(id));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('USER','ADMIN')")
    public ResponseEntity<ProdutoMarketplaceDto.ProdutoMarketplaceResponse> update(
        @PathVariable Long id,
        @Valid @RequestBody ProdutoMarketplaceDto.ProdutoMarketplaceRequest request
    ) {
        return ResponseEntity.ok(produtoMarketplaceService.update(id, request));
    }

    /**
     * Remove o registro alvo de forma controlada, respeitando regras e restricoes.
     */
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> delete(@PathVariable Long id) {
        produtoMarketplaceService.delete(id);
        return ResponseEntity.noContent().build();
    }
}
