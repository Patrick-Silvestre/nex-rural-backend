...
/**
 * Controller REST de Funcionario. Recebe requisicoes HTTP, valida dados de entrada e delega regras para a camada de servico.
 */
@RestController
@RequestMapping(ApiPaths.FUNCIONARIOS)
...
public class FuncionarioController {
    ...
    public ResponseEntity<FuncionarioDto.FuncionarioResponse> create(@Valid @RequestBody FuncionarioDto.FuncionarioRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(funcionarioService.create(request));
    }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('USER','ADMIN')")
    public ResponseEntity<FuncionarioDto.FuncionarioResponse> update(@PathVariable Long id,
                                                                     @Valid @RequestBody FuncionarioDto.FuncionarioRequest request) {
